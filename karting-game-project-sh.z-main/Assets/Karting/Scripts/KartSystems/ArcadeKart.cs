...
namespace KartGame.KartSystems
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        private float saveInterval = 5.0f;
        ...
        void Awake()
        {
            ...
            indoorBike = FTMS_IndoorBike.Instance; // Using singleton
        ...
        {
            Transform roadTransform = GetCurrentRoadTransform(); // Get the Transform of the current road
            if (roadTransform != null)
            {
                Vector3 roadCenter = roadTransform.position; // road center point
                Quaternion roadDirection = roadTransform.rotation; // road direction

                // Determine whether the track direction is negative
                if (Vector3.Dot(roadTransform.forward, Vector3.forward) < 0)
                {
                    // If the direction is negative, adjust the direction by 180 degrees
            ...
            {
                // If the current road information cannot be obtained, fall back to the previous saved state.
                RestorePlayerState();
            }
        }
