...
public class GameFlowManager : MonoBehaviour
{
    ...
    public bool autoFindBikes = true;
    //public ArcadeKart playerKart;
    public ArcadeBike playerBike;
    ...
    void Start()
    {
        // Start the coroutine to wait for the creation of the player's kart object
        // StartCoroutine(WaitForKartsSetup()); 
        StartCoroutine(WaitForBikeSetup());
        //if (autoFindKarts)
        if(autoFindBikes)
        {
            //karts = FindObjectsOfType<ArcadeKart>();
            bikes = FindObjectsOfType<ArcadeBike>();
            //if (karts.Length > 0)
            if (bikes.Length > 0)
            {
                //if (!playerKart) playerKart = karts[0];
                if (!playerBike) playerBike = bikes[0];

            }
            //DebugUtility.HandleErrorIfNullFindObject<ArcadeKart, GameFlowManager>(playerKart, this);
            DebugUtility.HandleErrorIfNullFindObject<ArcadeBike, GameFlowManager>(playerBike, this);
        }
    ...
    //IEnumerator WaitForKartsSetup()
    //{
    // // Wait until at least one ArcadeKart object is found
    //    while (FindObjectsOfType<ArcadeKart>().Length == 0)
    //    {
    //        yield return null; // wait for one frame
    //    }

    // // Now that we have ArcadeKart objects, we can get them and set them
    //    karts = FindObjectsOfType<ArcadeKart>();
    ...
    // // Start countdown and other logic
    //    ShowRaceCountdownAnimation();
    //    StartCoroutine(ShowObjectivesRoutine());
    //    StartCoroutine(CountdownThenStartRaceRoutine());
    //}
    IEnumerator WaitForBikeSetup()
    {
        // // Wait until at least one ArcadeKart object is found
        while (FindObjectsOfType<ArcadeBike>().Length == 0)
        {
        ...
        // // Now that we have ArcadeKart objects, we can get them and set them
        bikes = FindObjectsOfType<ArcadeBike>();
        foreach (ArcadeBike bike in bikes)
        {
            bike.SetCanMove(false);
        }
